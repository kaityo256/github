...
`git rebase`により歴史を改変すると、リモートとローカルで歴史が食い違ってしまう。すると、`git push`は差分の追加だけでリモートを更新することができない。

このような時のために、Gitにはforce pushというオプションが用意されている。

```sh
git push -f
```

と`-f`オプションを付けることで、無理やりローカルの歴史をリモートに反映させることができる。当然、同じリポジトリを共有している他の人は混乱することになるし、個人開発でも、複数のローカルリポジトリがあった場合には収拾つかなくなるであろう。

慣れるまでは、原則として

* プッシュ済みのブランチ(特に`main`や`master`)はリベースしない
