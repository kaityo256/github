...
Git管理前のプロジェクトがあったとする。Gitの初期化をすると、そこに空のインデックスとリポジトリが作られる。作られたばかりのリポジトリはこのプロジェクトのことを何も知らないので、教えてやらなければならない。最初の歴史を作るためには、まず管理するファイルをインデックスに登録する。これを**ステージ(stage)**、もしくは**ステージング(staging)** と言う。そのため、インデックスのことを **ステージングエリア(staging area)** とも呼ぶ。次に、インデックスに登録されたファイルを、リポジトリに登録する。これがコミットである。

リポジトリに登録されたことで、Gitはファイルの修正を追うことができるようになった。先ほど登録した状態から、ファイルを修正したとしよう。この修正をリポジトリに登録したい。そのために、まず修正したファイルをインデックスに登録する(ステージングする)。そして、コミットすることでリポジトリに登録できる。以後、これを繰り返すことで歴史を作っていくのがGitでの作業である。
