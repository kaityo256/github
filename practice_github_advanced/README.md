...
* レポート提出前に必ず`https://アカウント名.github.io/tyrano_sample`のURLで動作確認を行うこと。
* 最後に`git push`してから実際に反映されるまで数分かかる。
* `git push`する前に、必ず`git status`を実行し、`git add`忘れ、`git commit`忘れを確認すること。画像がGitリポジトリに追加されていない場合、ローカルでは問題なく動作するが、リモートでは画像が表示されない。

**ゲーム内容についての注意：**

* 公序良俗に反するような内容にしてはならない。
* (たとえ友人であっても)特定個人を揶揄するような内容にしてはならない。有名人も題材としない。
* 画像を用いる場合は、ライセンスとして問題ないものを利用する(例えば「[いらすとや](https://www.irasutoya.com/)」や「[Pexels](https://www.pexels.com/ja-jp/)」の画像を利用するなど)。
